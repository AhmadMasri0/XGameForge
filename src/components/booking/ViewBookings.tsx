...
const MyBookings = ({ rerender, setRerender }: MyBookingsProps) => {
    const [bookings, setBookings] = useState<Booking[]>([]);
    const [filtered, setFiltered] = useState<Booking[]>([]);
    const [tab, setTab] = useState<'upcoming' | 'inprogress' | 'past'>('upcoming');
    ...
    return (
        <Container maxWidth="md" sx={{ mt: 4 }}>
            ...
            <Tabs
                value={tab}
                onChange={(_: React.SyntheticEvent, val: 'upcoming' | 'inprogress' | 'past') => setTab(val)}
                sx={{
                    mb: 2,
                            ...
                            .map(b => (
                                <TableRow key={b._id}>
                                    ...
                                    <TableCell sx={{ textAlign: 'center' }}>
                                        {b.status === "upcoming" && (
                                            <Button color="error" onClick={() => handleCancel(b._id)} size="small">
                                                Cancel
                                            </Button>
                                        )}
                                    </TableCell>
