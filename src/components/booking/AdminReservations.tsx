...
import {
    ...
    TableSortLabel, TextField, Box, SelectChangeEvent
} from "@mui/material";
...
type SortConfig = {
    key: keyof Booking | "username" | "email" | "Cancel" | "Delete";
    direction: "asc" | "desc";
};
...
const AdminReservations: React.FC<Props> = ({ rerender, setRerender }) => {
    ...
    useEffect(() => {
        const fetchBookings = async () => {
            try {
                const res = await api.get<Booking[]>("/api/bookings/all");
    ...
    const getSortValue = (booking: Booking, key: string): string => {
        if (key === "username") return booking.user?.username || "";
        if (key === "email") return booking.user?.email || "";
        return booking[key as keyof Booking] as string;
    };
    ...
    const updateReservation = async (id: string | undefined, isCancel: boolean = false) => {
